...
export function SolutionPreviewPanel({ result, onClose, onSolver }: { result: SearchResult; onClose: () => void; onSolver: () => void }) {
  return (
    <div className="space-y-6">
        ...
        <ol className="space-y-3">
          {result.resolution_steps.map((step, index) => (
            <li className="flex gap-3 text-sm leading-6 text-text-muted" key={`${index}-${step}`}>
              <span className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full bg-brand-soft text-[10px] font-semibold text-brand-strong">{index + 1}</span>
              <span>{step}</span>
            </li>
          ))}
        </ol>
      ...
      <section>
        <h3 className="text-[10px] font-semibold uppercase tracking-[0.14em] text-text-muted">{copy.detail.technologies}</h3>
        <div className="mt-3 flex flex-wrap gap-2">
          {result.technologies.map((technology) => <span className="rounded-full border border-border bg-surface px-2.5 py-1 text-[11px] font-medium text-text-muted" key={technology}>{technology}</span>)}
        </div>
      </section>
...
function PreviewSection({ number, title, children }: { number: string; title: string; children: ReactNode }) {
  return <section className="grid gap-3 sm:grid-cols-[32px_minmax(0,1fr)]"><span className="pt-0.5 text-[10px] font-semibold text-brand-strong">{number}</span><div><h3 className="text-sm font-semibold text-text">{title}</h3><div className="mt-2">{children}</div></div></section>;
}
