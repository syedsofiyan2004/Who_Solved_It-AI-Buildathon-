...
export function SolutionPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="relative overflow-hidden rounded-[22px] border border-border bg-surface px-5 py-6 shadow-soft sm:px-8 sm:py-8">
        <div className="absolute -right-24 -top-28 h-64 w-64 rounded-full bg-primary/10 blur-3xl" />
        <div className="relative max-w-4xl">
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded-full bg-brand-soft px-2.5 py-1 text-[11px] font-semibold capitalize text-brand-strong">{item.status.replace(/_/g, " ")}</span>
            <span className="rounded-full border border-border bg-surface px-2.5 py-1 text-[11px] font-medium capitalize text-text-muted">{item.visibility}</span>
            {isVerified && <VerificationBadge verified date={item.last_verified_at ?? item.updated_at} />}
          </div>
          <h1 className="mt-4 max-w-3xl text-2xl font-semibold leading-tight tracking-[-0.035em] text-text sm:text-4xl">{item.title}</h1>
          <p className="mt-4 max-w-3xl text-sm leading-7 text-text-muted">{item.problem_description}</p>
          <div className="mt-5 flex flex-wrap items-center gap-2">
            {item.technologies.map((technology) => <span className="rounded-full border border-border bg-surface px-2.5 py-1 text-[11px] font-medium text-text-muted" key={technology}>{technology}</span>)}
          </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,820px)_320px] xl:items-start">
        <article className="min-w-0 rounded-[20px] border border-border bg-surface p-5 shadow-sm sm:p-8">
        ...
        <aside className="space-y-4 xl:sticky xl:top-[96px]">
          ...
          {item.related_solutions.length > 0 && <section className="rounded-[18px] border border-border bg-surface p-5 shadow-sm"><h2 className="text-sm font-semibold">{copy.detail.related}</h2><div className="mt-3 space-y-2">{item.related_solutions.map((related) => <Link className="block rounded-app border border-border p-3 text-sm transition-colors hover:border-border-strong hover:bg-surface-muted" key={related.challenge_id} to={`/solutions/${related.challenge_id}`}><span className="font-medium leading-5">{related.title}</span><span className="mt-2 block text-[11px] text-text-muted">{related.technologies.join(" · ")}</span></Link>)}</div></section>}
        </aside>
      </div>
    </div>
  );
...
function DocumentSection({ index, title, children }: { index: string; title: string; children: ReactNode }) {
  return <section className="grid gap-3 border-b border-border py-7 first:pt-0 last:border-0 sm:grid-cols-[42px_minmax(0,1fr)]"><span className="pt-0.5 text-[10px] font-semibold text-brand-strong">{index}</span><div className="min-w-0"><h2 className="text-sm font-semibold text-text">{title}</h2><div className="mt-3">{children}</div></div></section>;
}

function Meta({ icon: Icon, label, value }: { icon: LucideIcon; label: string; value: string }) {
  return <div className="flex items-start gap-3"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-control bg-surface-muted text-text-muted"><Icon className="h-4 w-4" /></span><div><p className="text-[10px] uppercase tracking-wide text-text-muted">{label}</p><p className="mt-0.5 text-sm font-medium capitalize text-text">{value}</p></div></div>;
}
