...
export function SearchPage() {
  ...
  const resultQuery = useQuery({
    ...
    queryFn: () => searchSolutions({
      query,
      filters: { verified_only: verifiedOnly },
      page,
      page_size: 10,
      sort,
    ...
    <form className="border border-border bg-surface p-4" onSubmit={submit}>
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <div className="relative flex-1">
          ...
          <input className="h-10 w-full rounded-control border border-border bg-surface pl-10 pr-3 text-sm placeholder:text-text-muted" id="solution-search" minLength={3} onChange={(event) => setInput(event.target.value)} placeholder={copy.search.placeholder} value={input} />
        </div>
        <Button disabled={input.trim().length < 3} type="submit" variant="primary">{copy.action.search}</Button>
      </div>
    </form>
    {!validQuery && <StatePanel kind="empty" />}
    {validQuery && <div className="grid gap-6 lg:grid-cols-[248px_minmax(0,1fr)]">
      <aside className="h-fit border border-border bg-surface p-4 lg:sticky lg:top-20">
        <div className="flex items-center gap-2"><SlidersHorizontal className="h-4 w-4 text-text-muted" aria-hidden="true" /><h2 className="text-sm font-semibold">{copy.search.filters}</h2></div>
        <label className="mt-4 flex min-h-11 items-center gap-2 text-sm text-text-muted"><input checked={verifiedOnly} onChange={(event) => setSearch({ verified: event.target.checked ? undefined : "false", page: undefined })} type="checkbox" />{copy.search.verifiedOnly}</label>
        <label className="mt-4 block text-sm text-text-muted" htmlFor="search-sort">{copy.search.sort}</label>
        <select className="mt-2 h-10 w-full rounded-control border border-border bg-surface px-2 text-sm" id="search-sort" onChange={(event) => setSearch({ sort: event.target.value === "newest" ? "newest" : undefined, page: undefined })} value={sort}><option value="relevance">{copy.search.sortRelevance}</option><option value="newest">{copy.search.sortNewest}</option></select>
        <Button className="mt-4 w-full" onClick={clear} variant="ghost">{copy.search.clear}</Button>
      </aside>
      <section aria-live="polite" className="min-w-0">
        <div className="mb-4 flex flex-col gap-3 border-b border-border pb-4 sm:flex-row sm:items-center sm:justify-between">
          <div><p className="text-sm font-medium text-text" data-testid="applied-search-query">{copy.search.appliedQuery} <span className="font-normal text-text-muted">&ldquo;{query}&rdquo;</span></p><p className="mt-1 text-sm text-text-muted">{verifiedOnly ? copy.search.verifiedResults : copy.search.resultCount}</p></div>
          <Button aria-pressed={includeSummary} onClick={() => setSearch({ summary: includeSummary ? undefined : "true", page: undefined })}>{copy.search.generateSummary}</Button>
        </div>
        {resultQuery.isLoading && <LoadingSkeleton rows={4} />}
        {resultQuery.isError && <StatePanel kind="error" onRetry={() => void resultQuery.refetch()} />}
        {response && <SearchResults response={response} onSelect={setSelected} onPage={(nextPage) => setSearch({ page: String(nextPage) })} />}
      </section>
    </div>}
...
function SearchResults({ response, onSelect, onPage }: { response: Awaited<ReturnType<typeof searchSolutions>>; onSelect: (result: SearchResult) => void; onPage: (page: number) => void }) {
  ...
  return <>
    {data.summary && <section className="mb-5 border border-border bg-surface p-4"><h2 className="text-sm font-semibold">{copy.search.summary}</h2><p className="mt-2 text-sm leading-6 text-text-muted">{data.summary}</p><div className="mt-3 flex flex-wrap gap-2"><span className="text-xs text-text-muted">{copy.search.sources}</span>{data.summary_citations.map((citation) => <code className="rounded-control border border-border px-2 py-1 text-xs" key={citation}>{citation}</code>)}</div></section>}
    {data.summary_error && <p className="mb-4 text-sm text-warning">{data.summary_error}</p>}
    <p className="mb-3 text-sm text-text-muted">{meta.total} {copy.search.resultCount}</p>
    <div className="space-y-3">{data.results.map((result) => <article className="border border-border bg-surface p-4 transition-colors hover:border-border-strong" key={result.solution_id}>
      <button className="w-full text-left" onClick={() => onSelect(result)}>
        <div className="flex flex-col justify-between gap-2 sm:flex-row"><h2 className="text-base font-semibold text-text">{result.title}</h2><div className="flex flex-wrap justify-end gap-1">{result.match_reasons.slice(0, 3).map((reason) => <span className="rounded-control border border-border px-2 py-1 text-xs text-text-muted" key={reason}>{reason}</span>)}</div></div>
        <p className="mt-2 text-sm leading-6 text-text-muted">{result.problem_excerpt}</p><p className="mt-3 text-sm text-text"><span className="text-text-muted">{copy.detail.solvedBy}: </span>{result.solver.display_name}</p>
        <div className="mt-3 flex flex-wrap gap-2">{result.technologies.map((technology) => <span className="rounded-control border border-border px-2 py-1 text-xs text-text-muted" key={technology}>{technology}</span>)}</div>
      </button>
      <Button className="mt-4" onClick={() => onSelect(result)}>{copy.action.viewSolution}</Button>
    </article>)}</div>
    <div className="mt-6 flex items-center justify-between"><Button disabled={meta.page <= 1} onClick={() => onPage(meta.page - 1)}><ChevronLeft className="h-4 w-4" aria-hidden="true" />{copy.search.previousPage}</Button><Button disabled={!meta.has_next} onClick={() => onPage(meta.page + 1)}>{copy.search.nextPage}<ChevronRight className="h-4 w-4" aria-hidden="true" /></Button></div>
  </>;
}
