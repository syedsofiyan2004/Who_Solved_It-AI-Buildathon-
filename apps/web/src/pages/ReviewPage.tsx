  ...
  return (
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-[22px] border border-border bg-surface p-6 shadow-soft sm:p-8">
        <div className="absolute -right-20 -top-24 h-60 w-60 rounded-full bg-primary/10 blur-3xl" />
        <div className="relative flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div><span className="inline-flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-brand-strong"><ShieldCheck className="h-3.5 w-3.5" />Quality control</span><h1 className="mt-2 text-2xl font-semibold tracking-[-0.035em] text-text sm:text-3xl">{copy.review.title}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-text-muted">Review the technical evidence, request precise changes, and publish only reusable solutions.</p></div>
          <div className="rounded-app border border-border bg-surface/75 px-4 py-3 text-sm backdrop-blur"><span className="text-2xl font-semibold tracking-[-0.03em] text-text">{queue.data?.length ?? 0}</span><span className="ml-2 text-xs text-text-muted">awaiting review</span></div>
        </div>
      </section>
      ...
      {!queue.data || queue.data.length === 0 ? <section className="rounded-[18px] border border-dashed border-border bg-surface p-10 text-center"><span className="mx-auto grid h-12 w-12 place-items-center rounded-app bg-brand-soft text-brand-strong"><CheckCircle2 className="h-5 w-5" /></span><h2 className="mt-4 text-lg font-semibold">Review queue is clear</h2><p className="mt-2 text-sm text-text-muted">{copy.review.empty}</p></section> : (
        <div className="grid gap-6 xl:grid-cols-[340px_minmax(0,1fr)]">
          <aside className="h-fit rounded-[18px] border border-border bg-surface p-3 shadow-sm xl:sticky xl:top-[96px]">
            <div className="p-2"><div className="flex items-center justify-between"><h2 className="text-sm font-semibold">{copy.review.queue}</h2><span className="rounded-full bg-brand-soft px-2 py-1 text-[10px] font-semibold text-brand-strong">{filteredQueue.length}</span></div><div className="relative mt-4"><Search className="absolute left-3 top-3 h-4 w-4 text-text-muted" /><input className="h-10 w-full rounded-control border border-input bg-surface pl-9 pr-3 text-sm outline-none focus:border-accent" id="review-filter" placeholder={copy.review.filter} value={filter} onChange={(event) => setFilter(event.target.value)} /></div><select className="mt-2 h-10 w-full rounded-control border border-input bg-surface px-3 text-sm outline-none focus:border-accent" id="review-sort" value={sort} onChange={(event) => setSort(event.target.value === "newest" ? "newest" : "oldest")}><option value="oldest">{copy.review.sortOldest}</option><option value="newest">{copy.review.sortNewest}</option></select></div>
            <div className="mt-2 space-y-1">{filteredQueue.length === 0 && <p className="px-3 py-6 text-center text-sm text-text-muted">{copy.review.noFilteredResults}</p>}{filteredQueue.map((item) => <button className={`w-full rounded-app border p-3.5 text-left transition-all ${item.id === activeId ? "border-primary/30 bg-brand-soft text-text shadow-sm" : "border-transparent text-text-muted hover:border-border hover:bg-surface-muted hover:text-text"}`} key={item.id} onClick={() => setSelectedId(item.id)} type="button"><span className="block text-sm font-semibold leading-5">{item.title}</span><span className="mt-2 flex items-center justify-between text-[10px]"><span className="capitalize">{item.visibility}</span><span>{new Date(item.updated_at).toLocaleDateString()}</span></span></button>)}</div>
          </aside>

          <section className="min-w-0 rounded-[20px] border border-border bg-surface p-5 shadow-sm sm:p-7">
            {detail.isLoading && <LoadingSkeleton rows={8} />}
            {detail.isError && <StatePanel kind="error" onRetry={() => void detail.refetch()} />}
            {detail.data && <>
              <div className="flex flex-col gap-4 border-b border-border pb-5 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex flex-wrap gap-2"><span className="rounded-full bg-brand-soft px-2.5 py-1 text-[10px] font-semibold capitalize text-brand-strong">{detail.data.status.replace(/_/g, " ")}</span><span className="rounded-full border border-border px-2.5 py-1 text-[10px] capitalize text-text-muted">{detail.data.visibility}</span></div><h2 className="mt-3 text-xl font-semibold tracking-[-0.025em] text-text">{detail.data.title}</h2></div><Link className="inline-flex h-9 items-center justify-center gap-2 rounded-control border border-border px-3 text-sm font-medium hover:bg-surface-muted" to={`/solutions/${detail.data.id}`}><FileSearch className="h-4 w-4" />{copy.action.viewSolution}</Link></div>
...
function ReviewSection({ title, body }: { title: string; body: string }) {
  return <section><h3 className="text-sm font-semibold">{title}</h3><p className="mt-2 whitespace-pre-wrap text-sm leading-7 text-text-muted">{body || "-"}</p></section>;
}
