...
import { ArrowRight, BrainCircuit, CheckCircle2, Clock3, DatabaseZap, FileClock, FilePlus2, GitBranch, Search, ShieldCheck, Sparkles, UsersRound } from "lucide-react";
...
export function DashboardPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="product-card overflow-hidden rounded-[24px] p-5 sm:p-6">
        <div className="absolute -right-20 -top-20 h-56 w-56 rounded-full bg-info/10 blur-3xl" />
        <div className="relative grid gap-6 xl:grid-cols-[280px_minmax(0,1fr)]">
          <div>
            <span className="inline-flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-brand-strong"><BrainCircuit className="h-3.5 w-3.5" />Showcase intelligence</span>
            <h2 className="mt-2 text-xl font-semibold tracking-[-0.03em]">Built to prove retrieval quality, not just page polish</h2>
            <p className="mt-2 text-sm leading-6 text-text-muted">The demo corpus is shaped like an enterprise knowledge base: repeated incidents across environments, verified owners, exact errors, technology tags, permission scopes, and AI-safe grounding boundaries.</p>
          </div>
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <ProofTile icon={DatabaseZap} label="Visible verified solutions" value={String(verifiedTotal)} body="Authorized runbooks returned from PostgreSQL." />
            <ProofTile icon={UsersRound} label="Expert profiles" value={people.isLoading ? "..." : String(people.data?.meta.total ?? 0)} body="Structured ownership and contacts, never generated by AI." />
            <ProofTile icon={GitBranch} label="Technology graph" value={technologies.isLoading ? "..." : String(technologies.data?.length ?? 0)} body="Filters and expertise are backed by normalized tags." />
            <ProofTile icon={ShieldCheck} label="Grounding guardrail" value="Cited" body="Summaries use only authorized top-ranked records." />
          </div>
        </div>
      </section>
...
function ProofTile({ body, icon: Icon, label, value }: { body: string; icon: LucideIcon; label: string; value: string }) {
  return <div className="interactive-lift rounded-[18px] border border-border bg-surface/80 p-4"><div className="flex items-center justify-between gap-3"><span className="grid h-9 w-9 place-items-center rounded-app bg-brand-soft text-brand-strong"><Icon className="h-4 w-4" /></span><span className="text-2xl font-semibold tracking-[-0.04em] text-text">{value}</span></div><p className="mt-4 text-[10px] font-semibold uppercase tracking-[0.14em] text-text-muted">{label}</p><p className="mt-2 text-xs leading-5 text-text-muted">{body}</p></div>;
}
