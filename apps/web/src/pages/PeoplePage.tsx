    ...
    <div className="space-y-6">
      <section className="product-card relative overflow-hidden rounded-[14px] p-6 sm:p-8">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div><span className="inline-flex items-center gap-2 font-data text-[10px] uppercase tracking-[0.14em] text-brand-strong"><UsersRound className="h-3.5 w-3.5" />Expert directory</span><h1 className="mt-2 font-display text-2xl font-semibold tracking-[-0.015em] text-text sm:text-3xl">Find the person behind the solution</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-text-muted">Search by name, role, team, or department and open the expert record connected to verified solutions.</p></div>
          <div className="relative w-full max-w-md"><Search className="absolute left-3.5 top-3.5 h-4 w-4 text-text-muted" /><input className="h-11 w-full rounded-control border border-input bg-surface pl-10 pr-3 text-sm outline-none transition-all duration-160 hover:border-border-strong focus:border-accent focus:shadow-focus" onChange={(event) => setQuery(event.target.value)} placeholder="Search by name, role, team, or department" value={query} /></div>
        </div>
      </section>
