    ...
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-[22px] border border-border bg-surface p-6 shadow-soft sm:p-8">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="relative w-full max-w-md"><Search className="absolute left-3.5 top-3.5 h-4 w-4 text-text-muted" /><input className="h-11 w-full rounded-app border border-input bg-surface pl-10 pr-3 text-sm outline-none focus:border-accent focus:shadow-focus" onChange={(event) => setQuery(event.target.value)} placeholder="Search by name, role, team, or department" value={query} /></div>
        </div>
      </section>
      ...
      {profiles.data && <>
        ...
        {profiles.data.data.length === 0 ? <section className="rounded-[18px] border border-dashed border-border bg-surface p-10 text-center"><h2 className="text-lg font-semibold">No people match that search</h2><p className="mt-2 text-sm text-text-muted">Try a department, technology, or shorter name.</p></section> : <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">{profiles.data.data.map((profile) => <article className="group rounded-[18px] border border-border bg-surface p-5 shadow-sm transition-all hover:-translate-y-px hover:border-border-strong hover:shadow-soft" key={profile.user_id}><div className="flex items-start gap-4"><div className="grid h-14 w-14 shrink-0 place-items-center rounded-[16px] border border-primary/20 bg-brand-soft text-base font-semibold text-brand-strong">{profile.initials}</div><div className="min-w-0"><h2 className="truncate text-base font-semibold text-text group-hover:text-brand-strong">{profile.display_name}</h2><p className="mt-1 line-clamp-2 text-sm leading-5 text-text-muted">{profile.job_title}</p><p className="mt-2 text-[11px] leading-5 text-text-muted">{profile.team}<br />{profile.department}</p></div></div><div className="mt-4 flex min-h-14 flex-wrap content-start gap-2">{profile.skills.slice(0, 4).map((skill) => <span className="rounded-full bg-surface-muted px-2.5 py-1 text-[10px] font-medium text-text-muted" key={skill}>{skill}</span>)}</div><div className="mt-5 flex gap-2 border-t border-border pt-4"><a aria-label={`Email ${profile.display_name}`} className="grid h-9 w-9 place-items-center rounded-control border border-border text-text-muted hover:bg-surface-muted hover:text-text" href={`mailto:${profile.contact_email}`}><Mail className="h-4 w-4" /></a><Link className="inline-flex h-9 flex-1 items-center justify-center gap-2 rounded-control border border-primary/25 bg-brand-soft px-3 text-sm font-medium text-brand-strong hover:bg-primary/10" to={`/people/${profile.user_id}`}>View profile<ArrowUpRight className="h-4 w-4" /></Link></div></article>)}</div>}
      </>}
    </div>
  );
}
