...
`.env` is ignored and is never committed. `.env.example` contains names and placeholders only. The base Compose file deliberately does **not** forward `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, or session tokens into the API container. Boto3 uses its standard credential provider chain: a local CLI profile when the optional `docker-compose.aws-profile.example.yml` override is selected, or the EC2 instance role when deployed. The override mounts the operator's existing `.aws` profile directory read-only and must never be used in EC2 deployment.
