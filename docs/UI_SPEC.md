...
| Colour tokens | `bg-canvas`, `bg-surface`, `bg-elevated`, `text-primary`, `text-secondary`, `text-muted`, `border-subtle`, `accent`, `accent-hover`, `focus-ring`, `success`, `warning`, `danger`, `info`, `code-bg`. No hex values until brand approval. |
...
| `/search` | Search results | Authenticated; `q` and filters in URL |
...
**Shell:** `AppShell`, `Sidebar`, `MobileNavSheet`, `TopBar`, `Breadcrumbs`, `CommandPalette`, `ThemeToggle`, `UserMenu`.

**Search:** `SearchInput`, `SearchFilters`, `FilterChip`, `SearchSummary`, `ResultList`, `SolutionResultCard`, `MatchReasonList`, `ResultPreviewDrawer`, `CitationList`, `NoAnswerPanel`.

**Knowledge:** `SolutionReader`, `SolutionMetadata`, `VerificationBadge`, `VisibilityBadge`, `CodeBlock`, `ResolutionStepList`, `AttachmentList`, `SolverPanel`, `FeedbackControl`.
...
| Search results and filters | 1120px two-column desktop: 248px sticky filter rail and results; mobile filter sheet then results. Query/URL synchronize. | Filter chips update URL and query; clear/filter keyboard accessible. Skeleton cards; blank query state; no-answer panel; network retry; result hover reveals preview action; disabled summary explains Bedrock state. | `SearchFilters`, `ResultList`, `SearchSummary`, `NoAnswerPanel`; `search.*`. |
| Result preview drawer | 560px right drawer on desktop, full-screen on mobile; summary, match reasons, root cause/steps excerpts, solver and source link. | Click card/Enter opens; Escape closes; focus returns to card. Loading skeleton; missing/denied becomes state panel; contact only when API supplies it. | `ResultPreviewDrawer`, `SolverPanel`, `CitationList`; `search.preview`, `action.view_solution`, `action.contact_solver`. |
