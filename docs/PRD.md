...
- Make search useful with keyword search before AI provider is available; clearly state the dependency state for semantic/RAG features.
...
| FR-07 | Generate grounded explanations | AI provider receives only permitted retrieved context and output includes record citations. |
...
- Security: controls in `SECURITY.md`; never embed personal contact data in vectors or pass disallowed content to AI provider.
...
PostgreSQL is authoritative for employee identity, contact data, ownership, roles, team/department, verification, and authorization. AI provider creates embeddings and grounded summaries only. It must not generate or infer structured employee/permission facts.
...
| 5 | Keyword search | FTS, exact error match, filters, sort, pagination, drawer and tests work without AI provider. |
| 6 | AI provider embeddings | Configured AI provider embedding adapter, hashes, re-embedding and failure states work without fake output. |
...
| 10 | Quality/deployment | Full test suite, local Docker workflow, end-to-end validation, and final product demonstration pass. |
